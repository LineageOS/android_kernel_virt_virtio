		...
		struct file *file, unsigned int flags, umode_t mode)
{
	...
	backing_dentry = lookup_one(&nop_mnt_idmap,
...
{
	struct fuse_file *ff = file->private_data;
	struct file *filp = ff->backing_file;
	int error;
	...
	error = filp->f_op->unlocked_ioctl(filp, command, arg);
			  ...
			  struct dentry *entry, unsigned int flags)
{
	...
	backing_entry = lookup_one(&nop_mnt_idmap,
			&(struct qstr) QSTR_INIT(entry->d_name.name,
...
int fuse_revalidate_backing(struct inode *dir, const struct qstr *name,
			    struct dentry *entry, unsigned int flags)
{
	struct fuse_dentry *fuse_dentry = get_fuse_dentry(entry);
	struct dentry *backing_entry = fuse_dentry->backing_path.dentry;
	struct inode *backing_dir = get_fuse_inode(dir)->backing_inode;
	...
	if (unlikely(backing_entry->d_flags & DCACHE_OP_REVALIDATE))
		return backing_entry->d_op->d_revalidate(backing_dir, name,
					backing_entry, flags);
	return 1;
}
		...
		struct inode *dir_inode, struct dentry *entry, umode_t mode)
{
	...
	if (!IS_POSIXACL(dir_backing_inode))
		mode &= ~fmi->umask;
	backing_path.dentry = vfs_mkdir(&nop_mnt_idmap, dir_backing_inode,
	...
	if (d_really_is_negative(backing_path.dentry) ||
		unlikely(d_unhashed(backing_path.dentry))) {
		struct dentry *d = lookup_one(&nop_mnt_idmap,
				&(struct qstr) QSTR_INIT(entry->d_name.name,
					entry->d_name.len),
			 ...
			 unsigned int flags)
{
	...
	rd = (struct renamedata) {
		...
		.old_parent = old_backing_dir_dentry,
		.old_dentry = old_backing_dentry,
		.new_parent = new_backing_dir_dentry,
